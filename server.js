...
io.on("connection", (socket) => {
    ...
    socket.on("callApi_map_car", async() => {
        socket.join("sercair")
        var xmls = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:web="http://webservice.com/">\n   <soapenv:Header/>\n   <soapenv:Body>\n      <web:PLITR1>\n                <a1>sercair</a1>\n                <a2>blink2014</a2>\n                <a3>213444</a3>\n                <o1>json</o1>\n      </web:PLITR1>\n   </soapenv:Body>\n</soapenv:Envelope>';
        var config = {
            method: 'post',
            ...
            headers: {
                ...
                'Cookie': 'FGTServer=94295B2F3C8A09563B8E86D4B0F2A2FC9F6C8C76D04B4426FBAE6A81F015BFB287C7',
            },
            data: xmls,
        }
        await axios(config).then(async function (response) {
            const res = JSON.parse(convert.xml2json(response.data, { compact: true, spaces: 2 }))
            const data = await res['soap:Envelope']['soap:Body']['ns2:PLITR1Response'].return._text;
